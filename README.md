...
Each round runs on a countdown that auto-reveals when it expires, so one AFK player can't stall everyone (the host also has a manual "Reveal now"). A player who drops mid-round can rejoin and still play, as long as they're back within a short grace window. A subtle cue (blip + vibration) marks each new round for players idling on the leaderboard. When the host ends the match, they choose a rematch — keeping the running scores or resetting them — or close the room.
